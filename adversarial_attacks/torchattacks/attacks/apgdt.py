...
class APGDT(Attack):
    ...
    def __init__(self, model, norm='Linf', eps=8/255, steps=100, n_restarts=1,
                 seed=0, eot_iter=1, rho=.75, verbose=False, n_classes=2):
    ...
    def dlr_loss_targeted(self, x, y, y_target):
        ...
        return -(x[np.arange(x.shape[0]), y] - x[np.arange(x.shape[0]), y_target]) / (x_sorted[:, -1] + 1e-12)
    ...
    def attack_single_run(self, x_in, y_in):
        ...
        if self.norm == 'Linf':
            ...
            x_adv = x.detach() + self.eps * torch.ones([x.shape[0], 1]).to(self.device).detach() * t / ((t ** 2).sum(dim=(1,), keepdim=True).sqrt() + 1e-12)
        ...
        step_size = self.eps * torch.ones([x.shape[0], 1]).to(self.device).detach() * torch.Tensor([2.0]).to(self.device).detach().reshape([1, 1])
        ...
        for i in range(self.steps):
            ...
            with torch.no_grad():
                ...
                if self.norm == 'Linf':
                    ...
                    x_adv_1 = x_adv + step_size[0] * grad / ((grad ** 2).sum(dim=(1,), keepdim=True).sqrt() + 1e-12)
                    x_adv_1 = torch.clamp(x + (x_adv_1 - x) / (((x_adv_1 - x) ** 2).sum(dim=(1,), keepdim=True).sqrt() + 1e-12) * torch.min(
                        self.eps * torch.ones(x.shape).to(self.device).detach(), ((x_adv_1 - x) ** 2).sum(dim=(1,), keepdim=True).sqrt()), 0.0, 1.0)
                    x_adv_1 = x_adv + (x_adv_1 - x_adv)*a + grad2*(1 - a)
                    x_adv_1 = torch.clamp(x + (x_adv_1 - x) / (((x_adv_1 - x) ** 2).sum(dim=(1,), keepdim=True).sqrt() + 1e-12) * torch.min(
                        self.eps * torch.ones(x.shape).to(self.device).detach(), ((x_adv_1 - x) ** 2).sum(dim=(1,), keepdim=True).sqrt() + 1e-12), 0.0, 1.0)
